...
const arraysEqual = (a1, a2) => {
    if (a1 && a1.length && a2 && a2.length) {
        return (a1.length === a2.length &&
            a1.every((o, idx) => deepEqual(o, a2[idx])));
    }
    return false;
} 
const runTests = () => {
    ...
    testResults.push({
      ...
      "tx"
        ...
        passed: arraysEqual(deleteOne([{ state: "tx" }, { state: "ny" }, { state: "ca" }, { state: "nm" }], "state", "tx"), [{ state: "ny" }, { state: "ca" }, { state: "nm" }]),
        result: JSON.stringify(deleteOne([{ state: "tx" }, { state: "ny" }, { state: "ca" }, { state: "nm" }], "state", "tx")),
    });
    return testResults;
} 
 runTests();